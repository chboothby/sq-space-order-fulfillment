...
@section('content')
    <div class="flex flex-col justify-center">
        ...
        <div class="mx-auto m-4 w-full bg-white p-6 rounded-lg  shadow-lg bg-white overflow-x-scroll">
            {{-- order details to be made into csv file --}}
            <form id="csv-form" action="{{ route('courier.make') }}" method="post" class="flex flex-col">
                        ...
                        @foreach ($orders as $order)
                            <tr class="border">
                                <td class="flex justify-around h-8 pt-4">
                                    <label for="{{ $order->order_number }}"
                                        class="self-center">#{{ $order->order_number }}</label>
                                    <input name="{{ $order->order_number }}" id={{ $order->order_number }}
                                        type="checkbox" checked class="self-center">
                                </td>
                ...
                @else<button onclick="return confirm('Please confirm you would like to create a csv');"
                        class="px-4 py-3 bg-theme text-white rounded-lg self-center mt-4" type="submit"
                        form="csv-form">Create CSV</button>
            </form>
            @endif
            {{ $orders->links() }}
        </div>
